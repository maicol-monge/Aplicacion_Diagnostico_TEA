...
const ActividadesADOS = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const res = await axios.get(`http://localhost:5000/api/ados/actividades/${modulo}`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
                setActividades(res.data);

                // 3. Cargar datos del paciente (para fecha de nacimiento)
                const pacienteRes = await axios.get(`http://localhost:5000/api/ados/paciente/${id_paciente}`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
                ...
                if (id_ados_url) {
                    ...
                    const obsRes = await axios.get(`http://localhost:5000/api/ados/actividades-realizadas/${id_ados_url}`, {
                        headers: { Authorization: `Bearer ${token}` }
                    });
    ...
    const guardarTestYObservaciones = async (estado) => {
        try {
            ...
            if (!id_ados) {
                ...
                const crearTest = await axios.post(`http://localhost:5000/api/ados/crear`, {
                    id_paciente,
                    modulo,
                    id_especialista,
                ...
                await axios.put(`http://localhost:5000/api/ados/pausar/${id_ados}`, { estado: 1 }, { headers: { Authorization: `Bearer ${token}` } });
            }

            // Guardar/actualizar observaciones
            for (const obs of observaciones) {
                await axios.post(`http://localhost:5000/api/ados/actividad-realizada`, {
                    id_ados,
    ...
    const handleFinalizarActividades = async () => {
        ...
        if (modulo === "1") {
            ...
            const codRes = await axios.get("http://localhost:5000/api/ados/codificacion/1", {
                headers: { Authorization: `Bearer ${token}` }
            });
            const puntRes = await axios.get("http://localhost:5000/api/ados/puntuaciones-codificacion/1", {
                headers: { Authorization: `Bearer ${token}` }
            });
