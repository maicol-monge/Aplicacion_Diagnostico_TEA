...
const TestsADOSPaciente = () => {
    ...
    useEffect(() => {
        const fetchTests = async () => {
            ...
            const res = await axios.get(`http://localhost:5000/api/ados/tests/${id_paciente}`, {
    ...
    const handleAplicarPuntuacionComparativa = async (id_ados) => {
        ...
        if (confirm) {
            ...
            try {
                ...
                await axios.put(`http://localhost:5000/api/ados/puntuacion-comparativa/${id_ados}`, {
    ...
    const handleDiagnosticar = async (id_ados) => {
        ...
        if (diagnostico) {
            ...
            try {
                ...
                await axios.put(`http://localhost:5000/api/ados/diagnostico/${id_ados}`, {
                    diagnostico
                }, { headers: { Authorization: `Bearer ${token}` } });
                                                ...
                                                {t.estado === 2 && (
                                                    <button
                                                        ...
                                                        onClick={async () => {
                                                            const token = localStorage.getItem("token");
                                                            try {
                                                                const res = await axios.get(`http://localhost:5000/api/ados/algoritmo-por-test/${t.id_ados}`, {
                                                                    headers: { Authorization: `Bearer ${token}` }
                                                                });
