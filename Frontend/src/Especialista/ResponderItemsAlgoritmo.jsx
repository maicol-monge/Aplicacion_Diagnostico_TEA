...
const ResponderItemsAlgoritmo = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const codRes = await axios.get(`http://localhost:5000/api/ados/codificaciones-algoritmo/${id_algoritmo}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            ...
            for (const cod of codRes.data) {
                const puntRes = await axios.get(`http://localhost:5000/api/ados/puntuaciones-codificacion/${cod.id_codificacion}`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
            ...
            const respRes = await axios.get(`http://localhost:5000/api/ados/respuestas-algoritmo/${id_ados}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            let respObj = {};
            ...
            const testRes = await axios.get(`http://localhost:5000/api/ados/test/${id_ados}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleGuardar = async () => {
        ...
        if (indice < codificaciones.length - 1) {
            ...
            const res = await axios.get(`http://localhost:5000/api/ados/puntuaciones-aplicadas/${id_ados}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            ...
            await axios.put(`http://localhost:5000/api/ados/clasificacion/${id_ados}`, {
                clasificacion,
                total_punto: suma
            }, { headers: { Authorization: `Bearer ${token}` } });
