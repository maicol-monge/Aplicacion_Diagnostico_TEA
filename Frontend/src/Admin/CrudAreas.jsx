...
const CrudAreas = () => {
    ...
    const fetchAreas = () => {
        axios.get("http://localhost:5000/api/admin/areas", { headers: { Authorization: `Bearer ${token}` } })
    ...
    const handleSubmit = e => {
        e.preventDefault();
        if (editId) {
            axios.put(`http://localhost:5000/api/admin/areas/${editId}`, form, { headers: { Authorization: `Bearer ${token}` } })
                .then(() => { setEditId(null); setForm({ area: "" }); fetchAreas(); });
        } else {
            axios.post("http://localhost:5000/api/admin/areas", form, { headers: { Authorization: `Bearer ${token}` } })
    ...
    const handleDelete = id =>
        ...
        axios.delete(`http://localhost:5000/api/admin/areas/${id}`, { headers: { Authorization: `Bearer ${token}` } }).then(fetchAreas);
