...
const CrudEspecialistas = () => {
    ...
    const fetchEspecialistas = () => {
        axios.get("http://localhost:5000/api/admin/especialistas", { headers: { Authorization: `Bearer ${token}` } })
    ...
    const handleSubmit = e => {
        e.preventDefault();
        if (editId) {
            axios.put(`http://localhost:5000/api/admin/especialistas/${editId}`, form, { headers: { Authorization: `Bearer ${token}` } })
                .then(() => { setEditId(null); setForm({ id_usuario: "", especialidad: "" }); fetchEspecialistas(); });
        } else {
            axios.post("http://localhost:5000/api/admin/especialistas", form, { headers: { Authorization: `Bearer ${token}` } })
    ...
    const handleDelete = id =>
        ...
        axios.delete(`http://localhost:5000/api/admin/especialistas/${id}`, { headers: { Authorization: `Bearer ${token}` } }).then(fetchEspecialistas);
