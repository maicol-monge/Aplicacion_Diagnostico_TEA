...
const CrudActividades = () => {
    ...
    const fetchActividades = () => {
        axios.get("http://localhost:5000/api/admin/actividades", { headers: { Authorization: `Bearer ${token}` } })
    ...
    const handleSubmit = e => {
        e.preventDefault();
        if (editId) {
            axios.put(`http://localhost:5000/api/admin/actividades/${editId}`, form, { headers: { Authorization: `Bearer ${token}` } })
                .then(() => { setEditId(null); setForm({ id_ados: "", nombre_actividad: "", observacion: "", puntuacion: 0 }); fetchActividades(); });
        } else {
            axios.post("http://localhost:5000/api/admin/actividades", form, { headers: { Authorization: `Bearer ${token}` } })
    ...
    const handleDelete = id =>
        ...
        axios.delete(`http://localhost:5000/api/admin/actividades/${id}`, { headers: { Authorization: `Bearer ${token}` } }).then(fetchActividades);
