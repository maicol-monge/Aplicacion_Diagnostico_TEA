...
const CrudPreguntas = () => {
    ...
    const fetchPreguntas = () => {
        axios.get("http://localhost:5000/api/admin/preguntas", { headers: { Authorization: `Bearer ${token}` } })
    ...
    const handleSubmit = e => {
        e.preventDefault();
        if (editId) {
            axios.put(`http://localhost:5000/api/admin/preguntas/${editId}`, form, { headers: { Authorization: `Bearer ${token}` } })
                .then(() => { setEditId(null); setForm({ pregunta: "", id_area: "" }); fetchPreguntas(); });
        } else {
            axios.post("http://localhost:5000/api/admin/preguntas", form, { headers: { Authorization: `Bearer ${token}` } })
    ...
    const handleDelete = id =>
        ...
        axios.delete(`http://localhost:5000/api/admin/preguntas/${id}`, { headers: { Authorization: `Bearer ${token}` } }).then(fetchPreguntas);
