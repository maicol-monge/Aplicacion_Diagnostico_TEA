...
const CrudUsuarios = () => {
    ...
    const fetchUsuarios = () => {
        axios.get("http://localhost:5000/api/admin/usuarios", { headers: { Authorization: `Bearer ${token}` } })
    ...
    const handleSubmit = e => {
        e.preventDefault();
        if (editId) {
            axios.put(`http://localhost:5000/api/admin/usuarios/${editId}`, form, { headers: { Authorization: `Bearer ${token}` } })
                .then(() => {
                    setEditId(null);
                    setForm({
                        nombres: "",
            ...
            axios.post("http://localhost:5000/api/admin/usuarios", form, { headers: { Authorization: `Bearer ${token}` } })
                .then(() => {
                    setForm({
                        nombres: "",
                        apellidos: "",
    ...
    const handleDelete = id =>
        ...
        axios.delete(`http://localhost:5000/api/admin/usuarios/${id}`, { headers: { Authorization: `Bearer ${token}` } }).then(fetchUsuarios);
