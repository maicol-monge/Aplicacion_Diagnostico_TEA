...
const CrudTestsAdiR = () => {
    ...
    const fetchTests = () => {
        axios.get("http://localhost:5000/api/admin/tests-adir", { headers: { Authorization: `Bearer ${token}` } })
    ...
    const handleSubmit = e => {
        e.preventDefault();
        if (editId) {
            axios.put(`http://localhost:5000/api/admin/tests-adir/${editId}`, form, { headers: { Authorization: `Bearer ${token}` } })
    ...
    const handleDelete = id =>
        ...
        axios.delete(`http://localhost:5000/api/admin/tests-adir/${id}`, { headers: { Authorization: `Bearer ${token}` } }).then(fetchTests);
